defmodule SetariaTest do
  ...
  test "hotp" do
    ...
    assert Setaria.valid_hotp?(expected_token, invalid_secret, counter, encoded_secret: false) ==
             false
  end

  test "totp" do
    timestamp = 1_332_083_784
    secret = "1234567890"
    encoded_secret = Base.encode32(secret, padding: false)
    invalid_secret = "1234567891"
    encoded_invalid_secret = Base.encode32(invalid_secret, padding: false)
    ...
    assert Setaria.valid_totp?(expected_token, secret, encoded_secret: false, timestamp: timestamp) ==
             true
    ...
    assert Setaria.valid_totp?(unexpected_token, secret,
             timestamp: timestamp,
             encoded_secret: false
           ) == false

    ## secret
    assert Setaria.valid_totp?(expected_token, encoded_invalid_secret, timestamp: timestamp) ==
             false

    assert Setaria.valid_totp?(expected_token, invalid_secret,
             timestamp: timestamp,
             encoded_secret: false
           ) == false
    ...
    assert Setaria.valid_totp?(expected_token, secret,
